# ---------- 평가 로그 ----------
        ...
        if self.target_gt and acc_tgt:
            ...
            acc_world_x = self._actor.get_location().x + acc_tgt.position_x
            acc_world_y = self._actor.get_location().y + acc_tgt.position_y
            dx = acc_world_x - gt_loc.x
            dy = acc_world_y - gt_loc.y
        ...
        self.log_rows.append({
            ...
            "ACC_Mode": acc_mode.name if acc_mode else "None",

            # Step 3 – GT 타겟 좌표 및 오차
            "Target_GT_X": round(gt_loc.x, 2),
            "Target_GT_Y": round(gt_loc.y, 2),
