self.log_data.append({
    ...
    # === Step 1~2: 감지 객체 수 및 내용 ===
    "num_objects_detected": len(obj_list),
    "num_objects_filtered": len(filtered),
    "num_objects_predicted": len(predicted),
    "filtered_object_ids": [f.object_id for f in filtered],
    "filtered_object_distances": [round(f.distance, 2) for f in filtered],
    "filtered_object_statuses": [f.status.name for f in filtered],
    ...
    "ls_lane_curvature": lane_data.curvature,
