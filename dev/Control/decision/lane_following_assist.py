...
MAX_STEER = 540.0
MIN_VEL = 0.1
SPEED_THRESHOLD = 16.67  # m/s (60 km/h)

# ===== FUNCTIONS =====
def lfa_mode_selection(ego):
    if ego is None or not math.isfinite(ego['velocity_x']):
        return LFAMode.LOW_SPEED
    return LFAMode.LOW_SPEED if ego['velocity_x'] < SPEED_THRESHOLD else LFAMode.HIGH_SPEED

def calculate_steer_in_low_speed_pid(lane, dt):
    ...
    if math.isinf(err):
        pid_integral = 0.0
        pid_prev_error = 0.0
        return math.copysign(MAX_STEER, err)
    ...
    if math.isinf(pid_integral) or abs(pid_integral) > 1e5:
        ...
        pid_integral = 0.0
        pid_prev_error = 0.0
        return 0.0 if abs(err) < 1e-6 else sign * MAX_STEER
    ...
    return max(min(out, MAX_STEER), -MAX_STEER)

def calculate_steer_in_high_speed_stanley(ego, lane):
    ...
    if math.isinf(hdg_err):
        return math.copysign(MAX_STEER, hdg_err)
    if abs(hdg_err) >= 180.0 and abs(cte) >= 2.0:
        return math.copysign(MAX_STEER, hdg_err + cte)
    ...
    return max(min(steer, MAX_STEER), -MAX_STEER)

def lfa_output_selection(mode, steer_pid, steer_stanley, lane, ego):
    ...
    if lane['is_curved_lane']:
        curve_gain = 1.2
        if ego['yaw_rate'] >= 30.0 or abs(ego['steering_angle']) >= 200.0:
            curve_gain = 0.8
        steer_out *= curve_gain

    return max(min(steer_out, MAX_STEER), -MAX_STEER)
